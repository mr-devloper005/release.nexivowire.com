    ...
    <EditableSiteShell>
      <main className="bg-[var(--slot4-page-bg)]">
        <section className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
          <section className="nexivo-card p-6 sm:p-8">
            <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
              ...
              <button type="button" className="nexivo-button-soft px-4 py-2 text-sm font-black" onClick={refreshComments}>Refresh comments</button>
            </div>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="relative w-full sm:max-w-md">
                ...
                <input
                  ...
                  className="nexivo-input h-11 w-full pl-9 pr-3 text-sm outline-none"
                />
              </div>
          ...
          {visibleComments.length ? (
            <section className="mt-8 grid gap-4">
              {visibleComments.map((item) => (
                <article key={`${item.articleSlug}-${item.id}`} className="nexivo-card p-5">
            ...
            <section className="mt-8 rounded-2xl border border-dashed border-[rgba(12,44,85,0.12)] bg-white p-8 text-center">
          ...
          {filtered.length > COMMENTS_PER_PAGE ? (
            <div className="mt-8 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[rgba(12,44,85,0.12)] bg-white p-4 text-sm text-[var(--slot4-muted-text)]">
              <span>Page {currentPage} of {totalPages}</span>
              <div className="flex gap-2">
                <button type="button" className="nexivo-button-soft px-4 py-2 font-black disabled:opacity-40" disabled={currentPage <= 1} onClick={() => setPage((value) => Math.max(1, value - 1))}>Previous</button>
                <button type="button" className="nexivo-button-soft px-4 py-2 font-black disabled:opacity-40" disabled={currentPage >= totalPages} onClick={() => setPage((value) => Math.min(totalPages, value + 1))}>Next</button>
              </div>
            </div>
          ) : null}
        </section>
