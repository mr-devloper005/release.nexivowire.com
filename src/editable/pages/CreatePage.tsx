...
const fieldClass = 'nexivo-input px-4 py-3 text-sm font-bold text-[var(--slot4-page-text)] outline-none placeholder:text-current/35'
...
export default function CreatePage() {
  ...
  if (!session) {
    return (
      <EditableSiteShell>
        <main className="nexivo-dark-shell min-h-screen px-4 py-16 sm:px-6 lg:px-8">
          <section className="mx-auto grid max-w-5xl gap-8 rounded-[2.8rem] border border-white/10 bg-white/10 p-7 text-white backdrop-blur md:grid-cols-[0.9fr_1.1fr] md:p-10">
            <div className="flex h-full min-h-72 items-center justify-center rounded-[2rem] bg-white/8 text-white">
            ...
            <div className="self-center">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <Link href="/login" className="nexivo-button px-6 py-3">Login <ArrowRight className="h-4 w-4" /></Link>
                <Link href="/signup" className="nexivo-button-soft border-white/15 bg-white/10 px-6 py-3 text-white">Sign up</Link>
              </div>
            </div>
          </section>
        </main>
    ...
    <EditableSiteShell>
      <main className="bg-[var(--slot4-page-bg)]">
        <section className="mx-auto max-w-[1320px] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 rounded-[2.8rem] border border-[rgba(12,44,85,0.12)] bg-white p-6 shadow-[0_30px_90px_rgba(12,44,85,0.1)] lg:grid-cols-[0.85fr_1.15fr] lg:p-10">
            <aside>
              <p className="text-xs font-black uppercase tracking-[0.28em] text-[var(--slot4-soft-muted-text)]">{pagesContent.create.hero.badge}</p>
              ...
              <div className="mt-8 grid gap-3 sm:grid-cols-2">
                {enabledTasks.map((item) => {
                  ...
                  return (
                    <button key={item.key} type="button" onClick={() => setTask(item.key)} className={`rounded-2xl border p-4 text-left transition ${active ? 'border-[var(--slot4-accent)] bg-[var(--slot4-accent)] text-white' : 'border-[rgba(12,44,85,0.12)] bg-white hover:-translate-y-0.5'}`}>
            ...
            <form onSubmit={submit} className="rounded-[2.2rem] border border-[rgba(12,44,85,0.12)] bg-[var(--slot4-page-bg)] p-5 sm:p-7">
              ...
              <button type="submit" className="nexivo-button mt-5 h-12 w-full">
