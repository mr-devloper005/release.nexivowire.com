...
const inputClass = 'nexivo-input h-[3.25rem] px-4 text-base font-bold text-black outline-none placeholder:text-black/35'
const buttonClass = 'nexivo-button h-[3.25rem] w-full'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-[1rem] border px-4 py-3 text-sm font-bold ${status === 'success' ? 'border-emerald-800/15 bg-emerald-50 text-emerald-800' : 'border-red-700/15 bg-red-50 text-red-700'}`}>{message}</p> : null}
